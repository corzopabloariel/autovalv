...
<script>
    window.familia_id = "{{ $data[ 'familia_id' ] }}";
    window.pyrus = new Pyrus( window.familia_id === null ? "familia" : "subfamilia" , null , src );
    window.elementos = @json($data["familia"]);
    ...
    familiasFunction = ( t , id ) => {
        window.location = `{{ URL::to( 'adm/familias/${id}/sub' ) }}`;
    };
    /** ------------------------------------- */
    init = ( callbackOK ) => {
        $( "#form .container-form" ).html(window.pyrus.formulario());
        $( "#wrapper-tabla > div" ).html(window.pyrus.table([ {NAME:"ACCIONES", COLUMN: "acciones", CLASS: "text-center", WIDTH:"150px"} ]));
        // , [ { icon : '<i class="fas fa-clipboard-list"></i>' , class: 'btn-dark' , function : 'familias' } ]
